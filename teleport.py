...
class Entities:
  ...
  def resource_attribute_eq(key: z3.SeqRef, *values: list[z3.SeqRef]) -> z3.BoolRef:
    return z3.And(
  ...
  def resource_attribute_str_eq(key: str, *values: list[str]) -> z3.BoolRef:
    return Entities.resource_attribute_eq(
      z3.StringVal(key),
      *[z3.StringVal(value) for value in values]
    )
  
  def resource_attribute_eq_any(key: z3.SeqRef) -> z3.BoolRef:
    return Entities._RESOURCE_ATTRIBUTE_KEYS(key)

  def resource_attribute_str_eq_any(key: str) -> z3.BoolRef:
    return Entities.resource_attribute_eq_any(z3.StringVal(key))
  ...
  def user_attribute_eq(key: z3.SeqRef, *values: list[z3.SeqRef]) -> z3.BoolRef:
    return z3.And(
  ...
  def user_attribute_str_eq(key: str, *values: list[str]) -> z3.BoolRef:
    return Entities.user_attribute_eq(
      z3.StringVal(key),
      *[z3.StringVal(value) for value in values]
    )
  
  USER_GROUP_MEMBERSHIP: z3.FuncDeclRef = z3.Function("user_group_membership", z3.StringSort(), z3.BoolSort())
